module.exports = (grunt) ->
	...
	taskConfig = 
		...
		clean:
			default: 'app/assets'
			dist: [
				'dist/snackk-web-api-login.min.js'
				'dist/snackk-web-api-login.min.js.map'
			]
		...
		requirejs:
			...
			api:
				options:
					mainConfigFile: 'api/assets/src/login-config.js'
					baseUrl: 'api/assets/src'
					out: 'dist/snackk-web-api-login.min.js'
					onBuildRead: (moduleName, path, contents) ->
						return contents.replace /\/assets/g, '/dist'
					include: ['../almond', 'login']
					exclude: ['jquery', 'lodash', 'jquery-cookie']
					wrap: {
